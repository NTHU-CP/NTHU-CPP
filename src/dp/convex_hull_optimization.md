...
- 插入：插入一條新的線
    ...
    則 \\(line1\\) 與 \\(line3\\) 交點的 \\(x\\) 座標為：
    \\(\frac{b_3-b_1}{a_1-a_3}\\)

    \\(line1\\) 與 \\(line2\\) 交點的 \\(x\\) 座標為：
    \\(\frac{b_2-b_1}{a_1-a_2}\\)

    因此，\\(line2\\) 要被刪除的條件為：
    \\(\frac{b_3-b_1}{a_1-a_3} \le \frac{b_2-b_1}{a_1-a_2}\\)
...
動態凸包，顧名思義就是動態維護的凸包，支援的操作有插入直線與詢問在 \\(x\\) 處的最大值。這個東西有模板可以用，應該蠻好找的，因此這邊不討論如何實作，只討論如何使用它。不過非常建議將動態凸包的模板放到 codebook 內，當遇到斜率優化的題目且斜率不單調時可以直接套。
