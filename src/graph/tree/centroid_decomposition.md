...
在性質3 的說明裡有提過，對於一個點 \\( x \\)，我們只需要對 \\(x\\) 所有重心樹上的祖先紀錄需要的資訊。
...
我們將 Fixed-Length Path 紀錄的 DP 資訊改成前綴和，並用資料結構(ex. BIT)來維護即可，但是目前這一題的重心剖分 + BIT解法可能會在最後幾筆測資TLE，想要通過的話可以用上面的 \\( O(N) \\) 作法。
...
假設現在我們現在要找出通過點 \\(u\\) 符合條件的路徑，就將 \\(u\\) 定為根，然後對其他連通的節點收集兩個資訊，一個是 \\(u \\) 到這個點的邊權和，二是到 \\(u \\) 的路徑長(邊的數量)。
...
首先，樹直徑可以用一個非常經典的樹DP求出，我們需要對於每個點計算從這個點出發往下走的 **最長** 和 **次長** 距離(兩條路徑不能有邊重疊)。我們可以根據這個做法下去延伸。
