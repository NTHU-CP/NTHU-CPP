...
時間複雜度為: \\( log(max\lbrace x, y \rbrace) \\)
...
遊戲會進行的回合數很多，B 能一直避開 A 的唯一方法只有在 A 兩側反覆橫跳。那麼 B 可以考慮待在樹上長度最長的鏈。令 A 每回合能動的步數為 \\( x \\)，B 的步數為 \\( y \\)。若 \\( y \leq 2x \\)，則 A 存在一種方法可以抓到 B。另外，若 \\( 2x >= 樹上長度最長的鏈 \\)，則 A 也可以抓到 B。證明的部分就請讀者自行練習。
