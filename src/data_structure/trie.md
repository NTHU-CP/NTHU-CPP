...
題目: 對於一個長度大於 \\(1\\) 的字串 \\(S = s*1s_1...s*{len}\\)，你可以做以下操作:

- 把 \\(S\\) 變成 \\(S = s*2s_3...s*{len}\\) (把第一個字元刪掉)
- 把 \\(S\\) 變成 \\(S = s*1s_3...s*{len}\\) (把第二個字元刪掉)
...
註: 對於一個字串 \\(S = s*1s_2...s*{len}\\)，\\(S[i]\\) 表示字元 \\(s*i\\)，\\(S[i,]\\) 表示後綴 \\(s_is_2...s*{len}\\)，\\(S[,i]\\) 表示前綴 \\(s*1s_2...s*{i}\\)，\\(S[i,j]\\) 表示子字串 \\(s*is_2...s*{j}\\)
