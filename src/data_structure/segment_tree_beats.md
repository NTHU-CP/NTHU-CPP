...
這些標記滿足每一個點的標記值都嚴格大於它子樹中的所有標記值。這個性質在轉換之後還會成立嗎？考慮轉換前後的差異：當 \\( mx2 \geq x \\) 時，我們會拜訪更多節點。換句話說，只有在子樹的所有標記都小於新標記，我們才打標記。
...
然後我們可以將一次區間最值操作所拜訪節點分成兩類：一類是本來就會拜訪的節點（底下稱為第一類），一類是由於更強的退出條件 \\( mx1 \leq x \\) 我們額外拜訪的節點（底下稱為第二類）。

接著定義權值 \\( \w(T) \\)（\\( T \\) 為標記類）為在子樹中擁有至少一個屬於標記類的 \\( tag \\) 的節點數，勢函數 \\( \Phi(x) \\) 為線段樹中所有標記類的 \\( \w(T) \\) 總和。

線段樹中最多存在 \\( n \\) 個標記，所以 \\( \Phi(x) \\) 的初始值是 \\( O(mlogn) \\)。考慮一次區間取 \\( max \\) 操作：

1. 考慮一次標記下傳，標記會下傳給兩子節點，所以我們讓 \\( \Phi(x) \\) 增加 \\( 2 \\)。
2. 新標記只會出現在第一類節點。當我們添加了一個新的標記類，它的權值等於我們打標記時經過的節點數，時間複雜度是 \\( O(logn) \\)。
3. 我們拜訪第二類節點的原因是我們必須刪除其子樹中的標記。如果我們刪除其子樹中屬於標記類 \\( T \\) 的一個 \\( tag \\)，子樹中所有屬於標記類 \\( T \\) 的 \\( tag \\) 都會被刪除，也就是回收之後這個點的子樹中一定不存在這一類標記。所以在拜訪每個第二類節點後，一定存在一類標記權值減少了 \\( 1 \\)，那麼 \\( \Phi(x) \\) 必定隨之減少了 \\( 1 \\)。
...
首先我們定義一種標記 \\( (a, b) \\)，這種標記是函數 \\( f_{a, b}(x) = max(x + a, b) \\)，表示給這個區間中的所有數先加上 \\( a \\)，然後再對 \\( b \\) 取 \\( max \\)。不難發現題目中涉及的三種操作都可以用這個標記來表示，分別為 \\( (-x, 0) \\)、\\( (x, -inf) \\)、\\( (-inf, x) \\)，\\( inf \\)是一個事先假設足夠大的數。
